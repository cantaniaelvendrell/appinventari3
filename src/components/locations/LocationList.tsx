...
export function LocationList() {
  ...
  const fetchLocations = async () => {
    try {
      ...
      console.error('Error al cargar les localitzacions:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Estàs segur que vols eliminar aquesta localització?')) return;
    ...
    try {
      ...
      console.error('Error en eliminar la localització:', error);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-48">
        <div className="text-xl">Carregant localitzacions...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-900">Localitzacions</h2>
        <button
          onClick={() => {
